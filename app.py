...
from PyQt5.QtWidgets import QApplication, QMainWindow, QAction, QLineEdit, QToolBar, QSizePolicy, QLabel, QGraphicsOpacityEffect, QMenuBar, QMenu, QFileDialog, QProgressDialog, QMessageBox, QInputDialog, QPushButton, QVBoxLayout, QWidget
...
class Browser(QMainWindow):
    ...
    def handle_download(self, download_item):
        ...
        if path:
            ...
            self.progress_dialog = QProgressDialog("Downloading...", "Pause", 0, 100, self)
            ...
            # Add resume button
            self.resume_button = QPushButton("Resume", self.progress_dialog)
            self.resume_button.hide()
            self.resume_button.clicked.connect(self.resume_download)
            self.progress_dialog.canceled.connect(self.pause_download)
            ...
            self.download_thread.progress_signal.connect(lambda received, total, elapsed: self.update_progress(self.progress_dialog, received, total, elapsed))
            self.download_thread.finished_signal.connect(lambda success: self.finish_download(self.progress_dialog, success))
    ...
    def pause_download(self):
        ...
        self.progress_dialog.setLabelText("Download paused")
